;; *discuss-pets*: development version 5 (UNDER CONSTRUCTION)
;;
;; After defining *discuss-pets*, we create a hash table 
;;       *output-semantics* 
;; containing interpretations of Lissa outputs, under hash keys 
;; like (*discuss-pets* ?e1). The main goal is to be able later
;; to match certain user inputs to question interpretations, to
;; see if the inputs already answer the questions, making them
;; redundant.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(store-schema 'discuss-pets-unconstrained.v

'(dial-schema :header (((set-of ^me ^you) discuss-pets-unconstrained.v) ** ?e)
;``````````````````````````````````````````````````````````````````````
...
)) ; END discuss-pets-unconstrained.v