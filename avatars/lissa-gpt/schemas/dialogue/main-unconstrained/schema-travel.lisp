;; *discuss-travel*: development version 5 (UNDER CONSTRUCTION)
;;
;; After defining *discuss-travel*, we create a hash table 
;;       *output-semantics* 
;; containing interpretations of Lissa outputs, under hash keys 
;; like (*discuss-travel* ?e1). The main goal is to be able later
;; to match certain user inputs to question interpretations, to
;; see if the inputs already answer the questions, making them
;; redundant.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(store-schema 'discuss-travel-unconstrained.v

'(dial-schema :header (((set-of ^me ^you) discuss-travel-unconstrained.v) ** ?e)
;````````````````````````````````````````````````````````````````````````
...
)) ; END discuss-travel-unconstrained.v