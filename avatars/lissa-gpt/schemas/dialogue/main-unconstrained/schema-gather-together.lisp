;; *discuss-gather-together*: development version 5 (UNDER CONSTRUCTION)
;;
;; After defining *discuss-gather-together*, we create a hash table 
;;       *output-semantics* 
;; containing interpretations of Lissa outputs, under hash keys 
;; like (*discuss-gather-together* ?e1). The main goal is to be able later
;; to match certain user inputs to question interpretations, to
;; see if the inputs already answer the questions, making them
;; redundant.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(store-schema 'discuss-gather-together-unconstrained.v

'(dial-schema :header (((set-of ^me ^you) discuss-gather-together-unconstrained.v) ** ?e)
;````````````````````````````````````````````````````````````````````````````````
...
)) ; END discuss-gather-together-unconstrained.v