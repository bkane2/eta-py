...
(store-schema 'have-eta-dialog-control.v

'(dial-schema :header (((set-of ^me ^you) have-eta-dialog-control.v) ** ?e)
;````````````````````````````````````````````````````````````````````````````
...
?e3 ((set-of ^me ^you) discuss-activities-unconstrained.v)

?e4 ((set-of ^me ^you) exchange-goodbyes.v)

)

)) ; END have-eta-dialog-control.v