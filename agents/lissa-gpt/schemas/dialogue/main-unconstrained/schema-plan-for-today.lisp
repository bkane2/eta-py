;; *discuss-plan-for-today*: development version 5 (UNDER CONSTRUCTION)
;;
;; After defining *discuss-plan-for-today*, we create a hash table 
;;       *output-semantics* 
;; containing interpretations of Lissa outputs, under hash keys 
;; like (*discuss-plan-for-today* ?e1). The main goal is to be able later
;; to match certain user inputs to question interpretations, to
;; see if the inputs already answer the questions, making them
;; redundant.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(store-schema 'discuss-plan-for-today-unconstrained.v

'(dial-schema :header (((set-of ^me ^you) discuss-plan-for-today-unconstrained.v) ** ?e)
;````````````````````````````````````````````````````````````````````````````````
...
)) ; END discuss-plan-for-today-unconstrained.v