...
def match(pa, ex, feats={}):
  ...
  def match_rec(pa, ex, feats):
    ...
    if listp(p):
      m = match_rec(p, e, feats)
      if ok(m):
        mm = match_rec(pa[1:], ex[1:], feats)
        if ok(mm):
          return cons(m, mm)
      return ':nil'
    ...
    if num_var(p):
      if int(p) == 0:
        return match_rec(cons('*expr', pa[1:]), ex, feats)
      elif int(p) == 1:
        return match_rec(cons('?expr', pa[1:]), ex, feats)
      else:
        mm = match_rec(pa[1:], ex, feats)
        if ok(mm):
          return cons([':seq'], mm)
        else:
          p = '?expr' if int(p) == 2 else dec_num_var(p)
          mm = match_rec(cons(p, pa[1:]), ex[1:], feats)
          if ok(mm):
    ...
    if p_var(p):
      ...
      if ok(m):
        ...
        mm = match_rec(cons(sv, pa[1:]), ex[1:], feats)
        if ok(mm):
          return cons(m + mm[0][1:], mm[1:])
      return ':nil'
